            ...
            <div>
                    {collection != null &&
                        <div key={collection.id} className="group cursor-pointer relative inline-block text-center">
                            {loading == true ?
                                <>
                                    <img id={collection.id} src={baseimg + collection.poster_path} alt={collection.name} className="rounded-3xl p-2 grayscale" />
                                    <div role="status" className="absolute z-10 left-1/2 right-1/2 top-1/2 bottom-1/2 m-auto">
                                ...
                                :
                                <>
                                    <button onClick={() => {setLoad(true); router.push("/collection/" + collection.id)}}>
