    ...
    return (
        <>
            <div className={"relative flex flex-col p-2 break-words shadow-soft-xl rounded-2xl bg-clip-border min-w-full" + height}>
                <div className="relative h-40 w-full overflow-hidden bg-cover rounded-xl bg-no-repeat bg-center hover:scale-105 transition" style={{backgroundImage: "url(" + itemdata.image + ")"}} 
                    onClick={() => {setMovieLoading(itemdata.itemname); router.push("/" + itemdata.type + "/" + itemdata.itemid)}}>
                    <span className="absolute top-0 left-0 w-full h-full bg-center bg-cover bg-gradient-to-tl from-gray-900 to-slate-800 opacity-80"></span>
                    {movie_loading == itemdata.itemname.toString() ?
                        <>
                            <div role="status" className="block z-10 m-auto my-14">
                        ...
                        <>
                            <div className="relative z-10 flex flex-col flex-auto h-full p-4">
                                ...
                                <h5 className="pt-2 mb-4 font-base text-white">{itemdata.comment}</h5>
                                {type == "rating" && 
                                    <p className="mt-auto mb-0 font-semibold leading-normal text-white group text-sm">
                                        {itemdata.rating}/10
                                    </p>
                                }
                                <p className="mt-auto pb-4 font-semibold leading-normal text-white group text-sm">{itemdata.added}</p>
                            </div>
                        </>
                    }
                </div>
